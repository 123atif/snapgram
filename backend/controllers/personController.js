...
} = require("../helperFunctions/tokens.js");
...
const registration = async (request, response) => {
  try {
    ...
    const personEmail = await Person.findOne({ email });
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in Registration" });
  }
};
...
const login = async (request, response) => {
  try {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in Login" });
  }
};
...
const updatePerson = async (request, response) => {
  try {
    ...
    if (person) {
      ...
      return response
        .status(200)
        .send({ status: true, message: "Updation Successful" });
    } else {
      return response
        .status(400)
        .send({ status: false, message: "Person NOT Found" });
    }
  } catch (error) {
...
const getPerson = async (request, response) => {
  try {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in Get Person Data" });
  }
};
...
const deletePerson = async (request, response) => {
  try {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in Deleting Person Data" });
  }
};
  ...
  try {
    ...
    if (!person) {
      ...
      if (indexPost >= 0) {
        ...
        return response
          .status(200)
          .send({ status: true, message: "Post Unsaved Successfully" });
      } else {
        person.saved.push(post._id);
        await person.save();
        return response
          .status(200)
          .send({ status: true, message: "Post Saved Successfully" });
      }
    }
  } catch (error) {
    console.error("Error!!!---------------", error);
    return response
      .status(400)
      .send({ status: false, message: "Error in Saving Post" });
  }
};
