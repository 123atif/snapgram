...
const addPost = async (request, response) => {
  try {
    ...
    console.log("request body user id--------", request.user.id);
    ...
    if (
      ...
      return response
        .status(400)
        .send({ status: false, message: "User Input Error" });
    } else if (!person) {
      return response
        .status(400)
      ...
      return response
        .status(200)
        .send({ status: true, message: "Post Uploaded Successfully" });
    }
  } catch (error) {
    console.log("Error add post-------------------", error);
    return response
      .status(400)
      .send({ status: false, message: "Error in Posting" });
  }
};
...
const updatePost = async (request, response) => {
  try {
    ...
    if (!postId) {
      return response
        .status(400)
        .send({ status: false, message: "Error! params id missing" });
    } else if (
      !caption ||
      !location ||
      !tags ||
      ...
      if (!person) {
        ...
        return response.status(400).send({
          status: false,
          message: "User have NO Access to update this post",
        });
      } else {
        let imageId, imageName;
        if (request.file) {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in updating Post" });
  }
};
...
const deletePost = async (request, response) => {
  try {
    ...
    if (!person) {
      ...
      return response.status(400).send({
        status: false,
        message: "User have NO Access to delete this post",
      });
    } else {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in deleting Post" });
  }
};
...
const getAllPosts = async (request, response) => {
  return response
    .status(200)
    .send({ status: true, message: "Get Post", data: await Post.find() });
};
const getPostById = async (request, response) => {
  try {
    ...
    if (!person) {
      ...
      return response.status(400).send({
        status: false,
        message: "User have NO Access to Get this post",
      });
    } else {
      console.log("Post--------------------", post);
      return response
        .status(200)
        .send({ status: true, message: "Get Post Successful", data: post });
    }
  } catch (error) {
    console.log("Error get post-------------------", error);
    return response
      .status(400)
      .send({ status: false, message: "Error in Get Post" });
  }
};
...
const likePost = async (request, response) => {
  try {
    ...
    return response
      .status(400)
      .send({ status: false, message: "Error in liking Post" });
  }
};
